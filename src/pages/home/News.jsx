...
function News() {
  ...
  return (
    <div>
        <h2 className='text-2xl font-semibold ml-3'>News</h2>
        <Swiper
        slidesPerView={1}
        spaceBetween={30}
        navigation={true}
        ...
        
    {
        news.map((item, index)=>(
            <SwiperSlide  key={index}>
                <div className='flex md:flex-row flex-col sm:justify-between items-center gap-12 mx-4 my-3'>
                    <div>
                        <Link to='/'>
                            <h3 className='text-2xl font-medium hover:text-blue-700 mt-1 '>{item.title}</h3>
                        </Link>
                        <div className='w-12 h-[4px] mb-5 bg-primary mt-1'></div>
